...
class StepCountJITAI(gym.Env):
  def __init__(self, sigma, chosen_obs_names,
               ...
               b_add_time_inhomogeneous=False, b_display=True):
    '''Class for StepCountJITAI environment.
    The possible obs names for chosen_obs_names are: 'C', 'P', 'L', 'H', 'D'... for example: chosen_obs_names=['C','H','D'], 
    where C is for true contex, L is for inferred context, H is for habituation, D is for disengagement.
    ...
    self.config = {'obs':self.chosen_obs_names_str, 'env_sigma':sigma, 'delta_h':self.δh, 'eps_h':self.εh, 'delta_d':self.δd, 'eps_d':self.εd,
                   'mu_s':self.µs, 'rho_1':self.ρ1, 'rho_2':self.ρ2 , 'rho_3':self.ρ3, 'D_threshold':self.D_threshold,
  ...
  def sample_context(self, chosen_rng, input_sigma, class_balance=.5):
    '''Generate a context sample (with 2 contexts). The output is (c_true, p, c_infer), where:
  ...
  def create_obs_array(self, input_c_true, input_probs, input_c_infer, input_h, input_d, input_z, input_w, input_s):
    """Create a full observation array from individual components."""
  ...
  def unpack_obs_array(self,obs):
    """Convert observation array into dictionary with named components."""
  ...
  def extract_obs(self, chosen_obs_names, b_add_time_inhomogeneous):
    '''Extract the obs in chosen_obs_names from the full state. If b_add_time_inhomogeneous is True,
  ...
  def extract_min_max(self, chosen_obs_names, b_add_time_inhomogeneous):
    '''Extract the min and max values for the obs in chosen_obs_names. If b_add_time_inhomogeneous is True,
  ...
  def get_inferred_error(self, chosen_sigma, b_default=True, sample_seed=0, N_data=1000):
    '''Compute the corresponding inferred error, given the uncertainty sigma.
